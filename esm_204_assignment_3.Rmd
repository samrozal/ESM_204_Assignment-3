...
demand_agg_new <- function(p){
  q <- demand_new_low(p, model_demand_l) + demand(p, model_demand_h)
  return(q)
}
...
ggplot(df, aes(Qagg_new, price_new)) +
  geom_line()
...
ggplot() +
  ...
  geom_line(df_2, mapping=aes(Qagg_new,price_new, color = "New Agg demand")) +
...
```{r}
xy <- intersect(demand_p_t_eq,mpc_eq)
```
