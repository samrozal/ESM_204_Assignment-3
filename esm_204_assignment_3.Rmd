...
p_tax_75 <- uniroot(equilibrium_pt_tax_75, c(0,600000))$root
p_tax_75
...
p_tax_150 <- uniroot(equilibrium_pt_tax_150, c(0,600000))$root
p_tax_150
...
```{r}
# divide this demand curve by 2
new_model_demand_h <- function(p,model){
new_model_demand_h <- (p - (0.5 * model$coefficients[1]))/ model$coefficients[2]
new_model_demand_h <- ifelse(q<0, 0, q)
  return(q)
}
model_demand_h
```
...
```{r}
#new agg demand function 
e_demand_agg <- function(p){
  e_q <- new_model_demand_h(p, model_demand_l) + new_model_demand_h(p, model_demand_h)
  return(e_q)
}
# how do I divide this model by 2
# new_model_demand_h <- lm(formula = (price_cents ~ q_high_kwh) * 0.5, data = df)
```

```{r}
#to find new equilibrium price set the functions equal to each other
uniroot_after_etax <- uniroot(function(p)
  e_demand_agg(p) - supply_q(p),
  interval = c(0, 20))

# I think we take the code from Q2 and and use the new model _ demand _ h curve I am trying to create in the code chunk above

# eqmQ = function(a, b, c){qsar = a/(b+c), return(qstart), newQ = eqmQ(newq, newb, newc)}
```
...
```{r}
@@ -615,7 +699,7 @@ uniroot_after_etax <- uniroot(function(p)
...
```{r}
# after we shift the demand curve by half, find MEC from scc (total env damage) under new demand curve, q * mec because area under the mec evaulated at q = total env damage, we are trying to find the tax that will make the two wedges (between MSC and MPC) the same? 
# I have no idea
```
