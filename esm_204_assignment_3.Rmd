...
- (5,100 cents/ 1 metric ton)(1 metric ton/2,205 lbs)(0.85lbs/1kWh)= 1.97 cents/kWh
...
demand_p_t <-function(q,model){
  p <- (model$coefficients[[1]])- 1.97 + (model$coefficients[[2]]*q)+
  return(p)
}
...
CS_new_75 <- function(p, model){
  q <- demand_new_75(p, model)
  cs <- 0.5*(model$coefficients[[1]] - p)*q
  return(cs)
}

CS_agg_75 <- function(p){
  cs <- CS_new_75(p,model_demand_l) + CS_new(p,model_demand_h)
  return(cs)
}

consumer_surplus_low_t_75 <- CS_new_75(p_tax_75,model_demand_l)
consumer_surplus_high_t_75 <- CS_new_75(p_tax_75,model_demand_h)
...
```{r}
#Write a function for rebate 
high_rebate <- q_tax_51
...
```{r}
# Use code from Q2 to but with new numbers from 6a 
```
