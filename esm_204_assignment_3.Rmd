...
```{r}
qseq = seq(0, 30, length.out = 100)
demand_low <- demand_indv(qseq, model_demand_l)

low_demand <- map(price,demand_low) %>% unlist()
df_2 <- tibble(demand_low = demand_low, price = price)

ggplot(df_2, aes(demand_low, price)) +
  geom_line()
...
```{r}
ggplot() +
  geom_line(df, mapping=aes(Qagg, price))+
  geom_line(df_2, mapping=aes(demand_low,price))
```
...
demand_low <- demand(p,model_demand_l)
