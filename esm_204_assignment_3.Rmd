...
differnce <- demand_agg_new_solar_p(q_solar)-demand_agg_p_new(q_solar)
differnce
...
```{r}
#Half the intercept of the demand function to lower the demand curve for the high income group by half.
demand_new_solar_2<- function(p, model){
  q <- (p - ((model$coefficients[[1]]))/model$coefficients[[2]]*.5)
  q <- ifelse(q<0,0,q)
  return(q)
}   

#new agg demand function accounting for solar
demand_agg_new_solar_2 <- function(p){
  q <- demand(p, model_demand_l) + demand_new_solar_2(p, model_demand_h)
  return(q)
}

equilibrium_pt_tax_solar_2 <- function(p)((demand(p, model_demand_l) + demand_new_solar_2(p, model_demand_h))-(p/mpc_slope))
p_solar_2 <- uniroot(equilibrium_pt_tax_solar_2, c(0,700000))$root

q_solar_2 <- demand_agg_new_solar_2(p_solar_2)
...
```
**C:** The total electricity environmental externality is $`r diff` cents.
